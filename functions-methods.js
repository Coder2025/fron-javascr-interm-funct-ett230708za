// Je gaat functies schrijven die we kunnen hergebruiken om sommige emailadressen te checken. Nu zul je gaan merken hoe
...
function getEmailDomain(emailAdres){
    return emailAdres.substring(emailAdres.indexOf('@')+1);
}
...
function getEmailType(emailAdres){
    let domeinType = '';
    let domeinEmail = emailAdres.substring(emailAdres.indexOf('@')+1);
    if (domeinEmail === 'novi-education.nl') {
        domeinType = 'Student';
    }else if (domeinEmail === 'novi.nl'){
        domeinType = 'Medewerker';
    }else{
        domeinType = 'Extern';
    }
    return domeinType;
...
console.log(getEmailType("n.eeken@gmail.nl"));
